...
2. ##### 회피(avoidance)
   ...
   알고리즘을 통해 해결하며 모두 프로세스의 실행 정보, 자원을 모두 알고 있어야 한다. 시뮬레이션을 통해서 결정을 내린다.
   ...
   > - 안정 상태?
   >     - 시스템의 모든 프로세스가 정상적으로 종료 가능한 '안전한 실행 순서'가 최소 하나 존재하는 상태.
   >     - 당장은 자원이 부족해 보여도, 특정 프로세스를 먼저 끝내고 자원을 반납하면, 그 자원으로 다른 프로세스를 끝내는 방식으로 모든 프로세스를 완료시킬 방법이 있는 상태를 의미.
