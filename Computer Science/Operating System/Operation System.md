...
TCP/IP 기반의 인터넷에 연결하거나, 응용 프로그램이 네트워크를 사용하려면 **운영체제에서 네트워크 프로토콜을 지원**해야 한다. 현재 상용 OS들은 다양하고 많은 네트워크 프로토콜을 지원한다. (TCP/IP, HTTP, SSL)
...
가장 대표적인 예로 다양한 nvidia의 그래픽 카드를 OS가 쉽게 인식하고 명령을 수행할 수 있도록 해주는 'nvidia 그래픽 드라이버'가 있다.

이러한 수많은 디바이스 드라이버를 관리하는 기능 또한 운영체제가 맡고 있다.

- 추상화 (Abstraction): 하드웨어의 복잡한 내부 동작을 숨기고, OS에게는 read(), write() 같은 단순하고 표준화된 인터페이스(API)를 제공한다.

- 통신 중계 (Translator): OS의 표준 명령을 하드웨어가 이해하는 언어로 번역하고, 하드웨어의 신호를 OS가 이해하는 언어로 번역한다.

- 자원 관리 (Resource Management): 하드웨어가 발생시키는 인터럽트(Interrupt)를 처리하고, 메모리 접근(DMA)을 관리하며, 하드웨어를 초기화하고 제어하는 역할을 수행한다.

특히, 디바이스 드라이버는 특정 OS에서 작동되도록 설계되기 때문에 OS 종속적이라고 볼 수 있다. 각종 드라이버를 다운로드 할 때 운영체제에 맞게 다운로드하는 것을 생각하면 된다.
